...
async function handleButtonClick(config, action, url, bannerID, containerID) {
    if (action === 'submit') {
        if (validateRequiredFields()) {
            ...
            config.uiSettings.fields.forEach(field => {
                if (field.type !== 'button') {
                    ...
                    if (field.type === 'radio') {
                        ...
                        uiPayload[field.crmFieldName] = checkedRadio ? checkedRadio.value : '';
                    } else if (field.type === 'checkboxGroup') {
                        ...
                        checkboxGroupData.forEach(e => {
                            if (e.checked) checkboxGroupPayload.push(e.value);
                        });
                        uiPayload[field.crmFieldName] = checkboxGroupPayload;
                    } else {
                        uiPayload[field.crmFieldName] = field.type === 'checkbox' ? inputElement.checked : inputElement.value;

                        // Check if this field needs validation and hasn't been verified yet
                        if (field.validate === true && inputElement.getAttribute('data-validate') === 'true' && inputElement.getAttribute('data-otp-verified') !== 'true') {
                            fieldToVerify = {
            ...
            // If a field needs verification, show modal
            if (fieldToVerify) {
                showOTPModal(fieldToVerify.value, fieldToVerify.type, bannerID, () => {
                    // On success, mark as verified and submit
                    fieldToVerify.element.setAttribute('data-otp-verified', 'true');
                    submitFormPayload(url, payload, containerID);
                });
            } else {
                // No verification needed, submit directly
                submitFormPayload(url, payload, containerID);
            }
        }
    } else if (action === 'next') {
...
// --- OTP Modal & Submission Helpers ---

/**
 * Submits the form payload to the server
 */
async function submitFormPayload(url, payload, containerID) {
    try {
        const response = await fetch(url, {
            method: 'POST',
            headers: {
...
/**
 * Inject generic modal styles if they don't exist
 */
function injectModalStyles() {
    if (document.getElementById('dcb-otp-modal-style')) return;
    const style = document.createElement('style');
    style.id = 'dcb-otp-modal-style';
    style.textContent = `
        .dcb-modal-overlay {
            position: fixed;
            top: 0;
 ...
 */
function showOTPModal(phoneOrEmail, type, bannerID, onSuccess) {
    injectModalStyles();

    // Create Modal HTML
    const overlay = document.createElement('div');
    overlay.className = 'dcb-modal-overlay';

    const content = document.createElement('div');
    content.className = 'dcb-modal-content';
    ...
    const title = document.createElement('div');
    title.className = 'dcb-modal-title';
    title.textContent = 'Verification Code';

    const subtitle = document.createElement('div');
    subtitle.className = 'dcb-modal-subtitle';
    subtitle.textContent = `We have sent the verification code to your ${type === 'email' ? 'email' : 'mobile number'}`;

    const otpContainer = document.createElement('div');
    otpContainer.className = 'dcb-otp-input-container';
    ...
    for (let i = 0; i < 6; i++) {
        ...
        input.type = 'text';
        input.maxLength = 1;
        input.className = 'dcb-otp-box';
        // Auto-focus logic
        input.onkeyup = (e) => {
            const key = e.key;
            if (key === 'Backspace' || key === 'Delete') {
                if (!input.value && i > 0) inputs[i - 1].focus();
        ...
        input.onpaste = (e) => {
            ...
            pastedData.forEach((char, index) => {
                if (index < 6) inputs[index].value = char;
            });
            if (pastedData.length > 0) inputs[Math.min(pastedData.length - 1, 5)].focus();
        };
    ...
    verifyBtn.className = 'dcb-btn-primary';
    verifyBtn.textContent = 'Verify';

    const errorMsg = document.createElement('div');
    errorMsg.className = 'dcb-error-msg';

    const resendLink = document.createElement('a');
    resendLink.className = 'dcb-resend-link disabled';
    resendLink.textContent = 'Resend Code in 60s';

    content.appendChild(closeBtn);
    content.appendChild(title);
    content.appendChild(subtitle);
    content.appendChild(otpContainer);
    content.appendChild(verifyBtn);
    content.appendChild(errorMsg);
    content.appendChild(resendLink);
    ...
    const sendOTPAPI = async () => {
        const payload = { dcb_id: bannerID };
        if (type === 'email') {
            payload.email = phoneOrEmail;
            payload.fieldType = 'email';
        } else {
            payload.phone_number = btoa('+91' + phoneOrEmail);
        }

        try {
            const res = await fetch(type === 'email' ? '/send_otp' : `${API_BASE_URL}dcb_send_otp`, {
                method: 'POST',
            ...
            errorMsg.textContent = 'Failed to send OTP. Please close and try again.';
        }
    };
    ...
    verifyBtn.onclick = async () => {
        ...
        if (type === 'email') {
            ...
            payload.phone_number = btoa('+91' + phoneOrEmail);
            payload.otp = btoa(otp);
        }

        try {
            const res = await fetch(type === 'email' ? '/verify_otp' : `${API_BASE_URL}dcb_verify_otp`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
            ...
            errorMsg.textContent = e.message || 'Verification failed';
            verifyBtn.textContent = 'Verify';
